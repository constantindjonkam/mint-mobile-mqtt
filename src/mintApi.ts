...
export async function fetchMintData(token: string, userId: string): Promise<MintAccountInfo[]> {
  ...
  // Resolve plan name
  const allAvailablePlans = [...(plansData?.availablePlans || []), ...(plansData?.phone?.availablePlans || [])];
      ...
      for (const member of activeMembers) {
        try {
          ...
          if (memberUsageRes.ok) {
            ...
            const mDataPercentUsed =
              mTotalMb > 0 ? +Math.min(100, Math.max(0, (mUsedMb / mTotalMb) * 100)).toFixed(2) : 0;
